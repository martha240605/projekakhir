...
@section('content')
    <div class="bg-white rounded-lg shadow-lg p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Edit Slot Waktu: {{ \Carbon\Carbon::parse($timeSlot->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($timeSlot->end_time)->format('H:i') }}</h2>
        ...
        <form action="{{ route('admin.time-slots.update', $timeSlot->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="time" id="start_time" name="start_time" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('start_time') border-red-500 @enderror" value="{{ old('start_time', \Carbon\Carbon::parse($timeSlot->start_time)->format('H:i')) }}" required>
            ...
            <div class="mb-4">
                ...
                <input type="time" id="end_time" name="end_time" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('end_time') border-red-500 @enderror" value="{{ old('end_time', \Carbon\Carbon::parse($timeSlot->end_time)->format('H:i')) }}" required>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="price" name="price" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('price') border-red-500 @enderror" value="{{ old('price', $timeSlot->price) }}" step="any">
            ...
            <div class="mb-6">
                ...
                <select id="is_available" name="is_available" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('is_available') border-red-500 @enderror" required>
                    <option value="1" {{ old('is_available', $timeSlot->is_available) == '1' ? 'selected' : '' }}>Ya</option>
                    <option value="0" {{ old('is_available', $timeSlot->is_available) == '0' ? 'selected' : '' }}>Tidak</option>
                </select>
